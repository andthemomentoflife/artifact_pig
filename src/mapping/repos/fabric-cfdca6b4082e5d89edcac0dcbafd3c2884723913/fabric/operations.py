...
def require(*keys, **kwargs):
    ...
    missing_keys = filter(
        lambda x: x not in env
        or (x in env and isinstance(env[x], (dict, list, tuple, set)) and not env[x]),
        keys,
    )
    if not missing_keys:
        return
        ...
        for lpath in names:
            try:
                if local_is_path and os.path.isdir(lpath):
                    p = ftp.put_dir(
                        lpath, remote_path, use_sudo, mirror_local_mode, mode
                    )
...
def _prefix_env_vars(command):
    ...
    if env_vars:
        exports = " ".join(
            '%s="%s"' % (k, _shell_escape(v)) for k, v in env_vars.iteritems()
        )
...
def _execute(
    ...
    with char_buffered(sys.stdin):
        ...
        workers = (
            ThreadHandler(
                "out", output_loop, channel, "recv", capture=stdout_buf, stream=stdout
            ),
            ThreadHandler(
                "err",
                output_loop,
...
def _run_command(
    ...
    with manager():
        ...
        result_stdout, result_stderr, status = _execute(
            default_channel(), wrapped_command, pty, combine_stderr, stdout, stderr
        )
...
def local(command, capture=False, shell=None):
    ...
    try:
        ...
        if shell is not None:
            ...
            p = subprocess.Popen(
                cmd_arg, shell=True, stdout=out_stream, stderr=err_stream
            )
...
def reboot(wait=120):
    ...
    with settings(hide("running"), timeout=timeout, connection_attempts=attempts):
        sudo("reboot")

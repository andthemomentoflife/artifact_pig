...
class Task(object):
    ...
    def __init__(
        self, alias=None, aliases=None, default=False, name=None, *args, **kwargs
    ):
...
def requires_parallel(task):
    ...
    * It's *not* been explicitly marked with ``@serial`` *and* the global
    ...
    """
    return (state.env.parallel and not getattr(task, "serial", False)) or getattr(
        task, "parallel", False
    )
...
def _parallel_tasks(commands_to_run):
    return any(
        map(lambda x: requires_parallel(crawl(x[0], state.commands)), commands_to_run)
    )
        ...
        for host in my_env["all_hosts"]:
            try:
                results[host] = _execute(
                    task, host, my_env, args, new_kwargs, jobs, queue, multiprocessing
                )

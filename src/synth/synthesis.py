...
def return_first_index(var, parent, node, nodeo, noden):
    ...
    elif hasattr(parent, "orelse") and (nodeo in parent.orelse or noden in parent.orelse):
        target = parent.orelse

    elif hasattr(parent, "finalbody") and (nodeo in parent.finalbody or noden in parent.finalbody):
...
def dec_to_other_syn(
    root: ast.AST,
    fcnode: Union[ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef],
    dnodes: set[ast.AST],
):
    def uvar_info(nodes):
        uvar = dict()
        ...
        for node in nodes:
            if isinstance(node, (ast.Assign)):
                for target in node.targets:
                    ...
                    NEN.visit(target)
                    try:
                        avar[node] = avar[node] | NEN.list
                    except:
                        avar[node] = NEN.list

            if isinstance(node, (ast.AugAssign, ast.AnnAssign)):
                NEN = call.NameExtractor()
                NEN.visit(node.target)
                avar[node] = NEN.list

        return avar

    def prioritize(dnodes: set[ast.AST]):
        assigns = [
            node
            for node in dnodes
        ...
        uvars = uvar_info(dnodes)  # key: node, value: set of used variables
        avars = avar_info(dnodes)  # key: node, value: set of assigned variables

        # Prioritize assign nodes
        ranking = []
        ranking_tmp = ranking
        ...
        for node in assigns:
            ...
            for r in ranking:  # r should be compared with node
                ruvar = uvars[r]
                ravar = avars[r]

                if len(avar & ruvar) > 0:
                    index = ranking.index(r)
                    ranking.insert(index, node)
                    break

            else:
                ranking_tmp.append(node)  # if no ...

        return assigns, etcs

    # Insert the nodes into the function (usually, decorator nodes located in very first part in the functions)
    class SynthDec(ast.NodeTransformer):
        def __init__(self, fcnode, anodes, enodes):
            self.fcnode = fcnode
            self.anodes = anodes
            self.enodes = enodes
        ...
        def visit_FunctionDef(self, node: ast.FunctionDef):
            if node == self.fcnode:
                node.body = self.anodes + self.enodes + node.body
    ...
    anodes, enodes = prioritize(dnodes)
    SDC = SynthDec(fcnode, anodes, enodes)
    h = SDC.visit(root)
...
def WithSynth(
    var, gp, wnode: Union[ast.With, ast.AsyncWith], nodeo=None
    ...
    def stmts(gp):
        ...
        try:
            if nodeo != None and nodeo in gp.body:
                stmts += [s for s in gp.body if check(var, s)]
        except:
            pass

        try:
            if nodeo != None and nodeo in gp.orelse:
                stmts += [s for s in gp.orelse if check(var, s)]
        except:
            pass

        try:
            if nodeo != None and nodeo in gp.finalbody:
                stmts += [s for s in gp.finalbody if check(var, s)]
        except:
            pass

        try:
            for handler in gp.handlers:
                if nodeo != None and nodeo in handler.body:
                    stmts += [s for s in handler.body if check(var, s)]
...
def FindDelNodes(lst, SurNode):
    class CFindDelNodes(ast.NodeVisitor):
        def __init__(self, news):
            if isinstance(news, ast.Assign):
                for target in news.targets:
            ...
            if node == self.new:
                ...
                for target in node.targets:
                    if isinstance(target, ast.Name) and target.id == self.target:
                        self.delnodes.add(node)

    result = set()
    for node in lst:
        FDN = CFindDelNodes(SurNode)
        FDN.visit(node)
        result = result.union(FDN.delnodes)
    return result

...
def is_total_import(root, var, libn) -> bool:
    stack = []
    exception_node = set()
    ...
    for dir in stack:
        dir_lib_path = dir_lib_path / Path(dir)
...
def get_accessible_py_path(patho: Path, py_path: str, level: int, lib: str):
    if patho.match("**/__init__.py") or patho.match("**/__init__.pyi"):
        _patho = patho.parent

    if py_path == None:
        _py_path = ""
    else:
        _py_path = py_path.replace(".", "/")

    _patho = patho

    if level > 0:
        for _ in range(level):
            _patho = _patho.parent
    else:
        _lib = llm_pre.libname(lib)
        _patho = Path(str(_patho).split(_lib)[0])

    path0 = _patho / (_py_path + ".py")
    path1 = _patho / _py_path
    path2 = _patho / _py_path / ".pyi"
    ...
    if check:
        ...
        if new_var in imports.keys():
            ...
            if isinstance(cmp, ast.ImportFrom):
                try: lib_name = cmp.module.split(".")[0]
                except: return (set(), {var})
            ...
            # Wrong lib match
                ...
                None, lib_name, libn
            ).ratio() > 0.8 and lib_name != llm_pre.libname(libn) and lib_name != libo:
            ...
            if llm_pre.libname(libn) not in ast.unparse(
                ...
                if lib_name == libo:
                    cmp_str = ast.unparse(cmp).replace(f' {libo}', f' {llm_pre.libname(libn)}') + f' as {llm_pre.libname(libo)}'
        ...
        for path, vals in apis.items():
            ...
            if v1 == path.split(".")[-1]:
                ...
                if path.count(".") > 0:
                    # from A.B import C
                    try: 
                        cand_paths.add((ast.ImportFrom, ".".join(path.split(".")[:-1]), path.split(".")[-1]))

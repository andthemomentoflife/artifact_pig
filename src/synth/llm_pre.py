...
def VarDict(new: dict, old: dict):
    ...
    for key, val in new.items():
        if key in new_dict.keys():
            new_dict[key] = new_dict[key].add(val)
        else:
            new_dict[key] = {val}
...
def libname(libo: str) -> str:
    total_loc = gits.git_loc
    total_loc.update(gits.git_loc_old)
...
def extract_var_map(nodeo, noden, codeo: ast.AST, coden: ast.AST, parento, b0=False):
    ...
    def filter(codeo: ast.AST, coden: ast.AST, v1, parento, nodeo) -> bool:
        ...
        fname = context_remover_refactor.nametarget(call.FindFCParent(parento, nodeo))
    ...
    if isinstance(nodeo, ast.ExceptHandler):
        for node in ast.walk(coden):
            if isinstance(node, ast.ExceptHandler):
                ...
                FCNN = context_remover_refactor.nametarget(FCN)
                FCNO = context_remover_refactor.nametarget(FCO)
    ...
    for noden in ast.walk(coden):
        ...
        if isinstance(noden, ast.Call):
            ...
            for (var, FCname), val in mappings.items():
                ...
                nameN = context_remover_refactor.nametarget(NFCP)

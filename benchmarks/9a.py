...
class _ApiClient(object):
    ...
    def __getattr__(self, item):
        if self._url:
            return _ApiClient(
                api_root=self._url + "/" + item, access_token=self._access_token
            )
    ...
    def __call__(self, *args, **kwargs):
        ...
        resp = requests.post(self._url, json=kwargs, headers=headers)
        if resp.ok:
            data = resp.json()
...
class CQHttp(_ApiClient):
    ...
    def send(self, context, message, **kwargs):
        if context.get("group_id"):
            return self.send_group_msg(
                group_id=context["group_id"], message=message, **kwargs
            )
        elif context.get("discuss_id"):
            return self.send_discuss_msg(
                discuss_id=context["discuss_id"], message=message, **kwargs
            )
        elif context.get("user_id"):
            return self.send_private_msg(
                user_id=context["user_id"], message=message, **kwargs
            )

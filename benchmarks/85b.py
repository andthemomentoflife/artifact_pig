...
class RemoteConfig(object):
    ...
    def getConfig(self):
        ...
        get(
            "%s/%s" % (self.remotepath, self.configfilename),
    ...
    def validationConfig(self):
        ...
        env.warn_only = True
        if (
            run("haproxy -c -f %s/%s" % (self.remotepath, self.configfilename)).find(
                "Configuration file is valid"
            )
            >= 0
        ):
            logger.debug(
                "[HAPROXY] remote configuration is valid, \
                                                              restart haproxy"
            )
            sudo("service haproxy restart")
            return True
...
class RemoteInterface(object):
    ...
    def addIP(self):
        if run("/sbin/ip addr show dev %s" % self.interface).find(self.IP) < 0:
            ...
            logger.debug(
                "[HAPROXY] remote add ip %s to inteface %s" % (self.IP, self.interface)
            )
        else:
            logger.debug(
                "[HAPROXY] remote ip %s is already configured on the \
                                              %s"
    ...
    def delIP(self):
        if run("/sbin/ip addr show dev %s" % self.interface).find(self.IP) >= 0:
            logger.debug(
                "[HAPROXY] remote delete ip %s from inteface %s"
                % (self.IP, self.interface)
            )
            sudo("/sbin/ip addr del %s/32 dev %s" % (self.IP, self.interface))
        else:
            logger.debug(
                "[HAPROXY] remote ip %s is not configured on the %s"
                % (self.IP, self.interface)
            )
        disconnect_all()
...
class RemoteSocketOperation(object):
    ...
    def __init__(self, device_ref, backend, rserver, interface, haproxy_socket):
    ...
    def suspendServer(self):
        out = sudo(
            "echo disable server %s/%s | socat stdio unix-connect:%s"
            % (self.backend_name, self.rserver_name, self.haproxy_socket)
        )
        if out == "":
            out = "ok"
        logger.debug(
            '[HAPROXY] disable server %s/%s. Result is "%s"'
    ...
    def activateServer(self):
        out = sudo(
            "echo enable server %s/%s | socat stdio unix-connect:%s"
            % (self.backend_name, self.rserver_name, self.haproxy_socket)
        )
        if out == "":
            out = "ok"
        logger.debug(
            '[HAPROXY] enable server %s/%s. Result is "%s"'
            % (self.backend_name, self.rserver_name, out)
        )
        disconnect_all()
    ...
    def getStatistics(self):
        ...
        out = sudo(
            ...
            " grep %s,%s " % (self.haproxy_socket, self.backend_name, self.rserver_name)
        )
        logger.debug(
            "[HAPROXY] get statistics about reserver %s/%s." " Result is '%s' ",
            self.backend_name,
            self.rserver_name,
            out,
        )

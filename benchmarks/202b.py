...
def login(database: Database) -> Dict[str, bool]:
    ...
    username = re.sub(
        unsafe_characters, "", credentials.get("username", "no username given")
    )
    ...
    try:
        ldap_server.simple_bind_s(
            f"cn={ldap_lookup_user},{ldap_root_dn}", ldap_lookup_user_password
        )
        ...
        ldap_server.simple_bind_s(
            f"cn={username},{ldap_root_dn}", credentials.get("password")
        )
    except (
